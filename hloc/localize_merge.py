...
def pose_from_cluster_(
    ...
    # from world-to-camera to camera-to-world
    t_retri = -R_retri.T @ t_retri
    R_retri = R_retri.T        

    # from rotation to euler
    r_retri = Rotation.from_matrix(R_retri)
    euler_retri = r_retri.as_euler('xyz', degrees=True)

    # 2) recover Rt from sensor
    Rt_sensor = sensor_dict[qname]

    R_sensor = Rt_sensor[0]
    t_sensor = Rt_sensor[1]

    # from world-to-camera to camera-to-world
    t_sensor = -R_sensor.T @ t_sensor
    R_sensor = R_sensor.T

    # from rotation to euler
    r_sensor = Rotation.from_matrix(R_sensor)
    euler_sensor = r_sensor.as_euler('xyz', degrees=True)

    # debug
    # 3) recover Rt from gt pose
    Rt_gt = gt_dict[qname]
    ...
    # euler_retri_dif = euler_gt - euler_retri
    # t_retri_dif = t_gt - t_retri        

    # euler_sensor_dif = euler_gt - euler_sensor
    # t_sensor_dif = t_gt - t_sensor            
    
    # print (qname, 'retri rotation ', euler_retri_dif)
    # print (qname, 'retri translation ', t_retri_dif)
    # print (qname, 'sensor rotation ', euler_sensor_dif)
    # print (qname, 'sensor translation ', t_sensor_dif)
    # import ipdb; ipdb.set_trace();


    # 4) merge Rt from retrieval and sensor results    
    # merge rotation
    euler_merge = np.zeros(3)
    euler_merge[:2] = euler_sensor[:2]
    euler_merge[2] = euler_retri[2]

    # merge translation
    t_merge = np.zeros(3)
    t_merge_dif = t_retri[:2] - t_sensor[:2]
    if np.absolute(t_merge_dif).max() > 10:
        t_merge[:2] = t_sensor[:2] # assume GPS precision is within 10 meters
    else:
        t_merge[:2] = 1/2 * (t_retri[:2] + t_sensor[:2])
    t_merge[2] = t_retri[2] # photos are captured at nearly the same height

    # euler_merge_dif = euler_gt - euler_merge
    # t_merge_dif = t_gt - t_merge           

    # print (qname, 'rotation ', euler_merge_dif)
    # print (qname, 'translation ', t_merge_dif)

    r_merge = Rotation.from_euler('xyz', euler_merge, degrees=True)
    R_merge = r_merge.as_matrix()

    # from camera-to-world to world-to-camera 
    t_merge = -R_merge.T @ t_merge
    R_merge = R_merge.T     

    qvec_merge = rotmat2qvec(R_merge)

    return qvec_merge, t_merge
