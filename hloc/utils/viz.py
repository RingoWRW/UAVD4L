...
def plot_matches(kpts0, kpts1, color=None, lw=1, ps=4, indices=(0, 1), path= None,  a=1.):
    ...
    #     fig.lines += [matplotlib.lines.Line2D(
    #         (fkpts0[i, 0], fkpts1[i, 0]), (fkpts0[i, 1], fkpts1[i, 1]),
    #         zorder=1, transform=fig.transFigure, c='lime', linewidth=lw,
    #         alpha=a)
    #         for i in range(len(kpts0))]
    ...
    if ps > 0:
        ...
        ax0.scatter(kpts0[:, 0], kpts0[:, 1], c=C, cmap=cm.hsv,s=ps)
        ax1.scatter(kpts1[:, 0], kpts1[:, 1], c=C, cmap=cm.hsv,s=ps)
    if path is not None:
