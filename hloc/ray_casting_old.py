...
def DSM2npy (DSM_path, save_path):
...
def read_DSM_config(args):
    DSM_path = args["DSM_path"]
    DSM_npy_path = args["DSM_npy_path"]
    num_sample = args["num_sample"]
    area_minZ = args["area_minZ"]
    ...
    dataset = None
    return geotransform, area, num_sample, area_minZ, write_path
...
def main(args, 
         ...
         object_dict: Dict):
    geotransform, area, num_sample, area_minZ, write_path = read_DSM_config(args)
    ...
    for name in names:
        K = K_w2c[name]
        pose = poses[name]
        z_name = name.split('_W')[0]
        
        #判断dict名字并warning
        photo_num = z_name.split('_')[-1]
        target_time = datetime.datetime.strptime(z_name.split('_')[1], '%Y%m%d%H%M%S')
