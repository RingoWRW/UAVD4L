...
DEFAULT_DIST_THRESH = 10  # in meters
...
def get_db_retrieval_pose(images):
    ...
    # flip to DEVICE coordinate system, which is 
    # different from standard OPENCV camera coordinate (y/z direction).
    Rs[:, :, [1,2]] = - Rs[:, :, [1,2]]
...
def main(descriptors, output, num_matched,
    ...
    logger.info('Extracting image pairs from a retrieval database with GT pose prior.')
