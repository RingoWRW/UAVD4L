...
confs = {
    ...
    'svcnn': {
        'output': 'feats-svcnn',
        # 'output_query': 'feats-svcnn_query',
        'model': {
            'name': 'svcnn'
        },
        'preprocessing': {
...
def main(conf: Dict,
    ...
    logger.info('Extracting global features with configuration:'
                f'\n{pprint.pformat(conf)}')
    slogen = 'db/'
    dataset = ImageDataset(image_dir, conf['preprocessing'], image_list)
    if feature_path is None:
        feature_path = Path(export_dir, 'db_' + conf['output']+'.h5')
    feature_path.parent.mkdir(exist_ok=True, parents=True)
    skip_names = set(list_h5_names(feature_path)
                     if feature_path.exists() and not overwrite else ())
    dataset.names = [n for n in dataset.names if n not in skip_names]
    if len(dataset.names) == 0:
        logger.info('Skipping the reference global feature extraction.')
        return feature_path
