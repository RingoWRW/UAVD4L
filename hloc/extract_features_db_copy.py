...
def main(
    ...
    dataset.names = [n for n in dataset.names if slogen + n not in skip_names]
    ...
    for idx, data in enumerate(tqdm(loader)):
        ...
        depth_name = name.split('/')[-1].split('.')[0] + '0046.exr'
        ...
        # print(pred['keypoints'][valid].shape, depth.shape,'--------------------------------------------', pred['keypoints'])
            ...
            for k, v in pred.items():
                if k in 'descriptors':
                    ...
                    pred[k] = v
            # pred = {k: (v[0])[valid].cpu().numpy() for k, v in pred.items()}
