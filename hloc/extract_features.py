...
confs = {
    ...
    'superpoint_max': {
        'output': 'feats-svcnn',
                'output_db': 'db_feats-svcnn',
        'output_query': 'query_feats-svcnn',
        'model': {
            'name': 'superpoint',
            'nms_radius': 3,
            'max_keypoints': 4096,
    ...
    'sift': { 
        'output_db': 'db_feats-sift',
        'output_query': 'query_feats-sift',
        'output': 'feats-sift',
        'model': {
            'name': 'dog'
        },
...
def main(conf: Dict,
    ...
    for idx, data in enumerate(tqdm(loader)):
        # time1 = time.time()
        # print("local feature preprocessing:", time1 - t_start)
        name = dataset.names[idx]
        pred = model({'image': data['image'].to(device, non_blocking=True)})
        pred = {k: v[0].cpu().numpy() for k, v in pred.items()}
        # time2 = time.time()
        # print("local feature extraction:", time2 - time1)
        ...
        del pred
        t_end = time.time()
        print("local feature save:", t_end - time2)
